...
async function setup() {
  ...
  const dbScriptSuccess = runCommand('npm run setup:db', 'Database structure fix');
  if (!dbScriptSuccess) {
  ...
  runCommand('npm run setup:auth', 'Auth database fix');
  
  // 4. Cleanup unnecessary files
  runCommand('npm run setup:cleanup', 'Cleanup of unnecessary files');
  ...
  console.log(`To create your first admin user, run: npm run admin:create <email>`);
