...
// This route fixes the database error: "ERROR: relation \"subscriptions\" does not exist"
// which occurs during authentication processes like magic link recovery
export async function GET(req: NextRequest) {
  try {
    ...
    // SQL to create the subscriptions table if it doesn't exist
    ...
    // Also make sure we have the recovery_sent_at column in profiles
    const updateProfilesTable = `
      -- Make sure profiles has recovery_sent_at column
      ALTER TABLE profiles
      ADD COLUMN IF NOT EXISTS recovery_sent_at TIMESTAMPTZ DEFAULT NULL;
    `;

    const { error: profilesError } = await supabaseAdmin.rpc('pgrest_sql', { query: updateProfilesTable });
    
    if (profilesError) {
      console.error('❌ Error updating profiles table:', profilesError);
      return NextResponse.json({ 
        error: `Database error updating profiles: ${profilesError.message}`,
      }, { status: 500 });
    }
    
    return NextResponse.json({
      success: true,
      message: 'Auth relations fixed successfully'
    });
    
  } catch (error) {
    console.error('❌ Error in fix-auth-relations endpoint:', error);
    return NextResponse.json({
