...
export function useTeamPortfolio() {
  ...
  const addCoin = async (coinData: CoinData, amount: number) => {
    if (!user) {
      toast({
        title: 'Error',
  ...
  const updateAmount = async (itemId: string, amount: number) => {
    if (!user) {
      toast({
        title: 'Error',
        description: 'You must be logged in to perform this action',
  ...
  const removeCoin = async (itemId: string) => {
    if (!user) {
      toast({
        title: 'Error',
        description: 'You must be logged in to perform this action',
  ...
  return {
    ...
    error,
    isAdmin,
    refreshPortfolio: fetchTeamPortfolio,
    addCoin,
    updateAmount,
    removeCoin
  };
