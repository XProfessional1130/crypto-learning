...
export async function getTeamPortfolio(): Promise<PortfolioSummary> {
  try {
    ...
    // For demonstration purposes, just fetch all portfolio items (or none)
    // In a real implementation, you'd filter by the admin user's ID
    ...
    if (!portfolioItems || portfolioItems.length === 0) {
      console.log('No portfolio items found. Please add items through the user interface.');
